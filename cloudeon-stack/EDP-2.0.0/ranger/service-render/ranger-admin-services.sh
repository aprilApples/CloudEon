...
if [[ ${JAVA_OPTS} != *"-Duser.timezone"* ]] ;then  export JAVA_OPTS=" ${JAVA_OPTS} -Duser.timezone=UTC+8" ;fi

# jmx
JAVA_OPTS=" ${JAVA_OPTS} -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.local.only=false -Dcom.sun.management.jmxremote.port=9605 -javaagent:/opt/jmx_exporter/jmx_prometheus_javaagent.jar=9505:${XAPOLICYMGR_DIR}/jmx_prometheus.yaml"
