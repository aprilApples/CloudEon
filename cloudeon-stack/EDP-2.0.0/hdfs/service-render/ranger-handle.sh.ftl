...
if [ "$RANGER_ENABLE" = "true" ]; then
  ...
  cp -f /opt/service-render-output/ranger-install.properties $RANGER_HOME/ranger-hdfs-plugin/install.properties
  cp -f /opt/service-render-output/ranger-xasecure-audit.xml $RANGER_HOME/ranger-hdfs-plugin/install/conf.templates/enable/xasecure-audit.xml
  bash $RANGER_HOME/ranger-hdfs-plugin/enable-hdfs-plugin.sh
else
  echo "Disable Ranger."
fi
